...
def main():
    ...
    token_for_sale = GovernableERC20.deploy(
        ...
        "DummyToken",
        "TKN",
        constants.AMOUNT_FOR_SALE + 10e18,
        {"from": accounts[0]},
    )

    token_for_sale.approve(
        factory.address, constants.AMOUNT_FOR_SALE + 100e18, {"from": accounts[0]}
    )
    ...
    for acc in accounts[1:10]:
        dai.approve(launch.return_value, 1000e18, {"from": acc})
        BasicLaunch.at(launch.return_value).sendUSD(1000e18, {"from": acc})
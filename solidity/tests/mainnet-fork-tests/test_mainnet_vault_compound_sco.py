...
def test_deposit_to_all_funds_withdrawn(successful_launch, accounts, cdai, deployed_factory, comp):
    ...
    launch_contract.withdrawComp({"from": accounts[0]})
    assert comp.balanceOf(accounts[0]) > 0
...
def test_exit_vault_comp(success_launch_comp, accounts, cdai, send_10_eth_of_dai_to_accounts, deployed_factory, comp):
    ...
    success_launch_comp.withdrawComp({"from": accounts[0]})
    assert comp.balanceOf(success_launch_comp) > 0

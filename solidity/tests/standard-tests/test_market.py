...
def test_deployment_should_configure(running_launch, accounts, deployed_factory):
    ...
    assert market_contract.ownershipSet({'from': accounts[0]})
...
def test_should_reject_if_set_bid_shares_from_non_venture_bond(running_launch, accounts, deployed_factory, send_any_usd_to_accounts):
    market_contract = brownie.Market.at(running_launch.launchMarketAddress({'from': accounts[0]}))
...
def test_should_reject_if_set_ask_from_non_venture_bond(running_launch, accounts, deployed_factory, send_any_usd_to_accounts):
    market_contract = brownie.Market.at(running_launch.launchMarketAddress({'from': accounts[0]}))
    with brownie.reverts("Market: Only VentureBond contract"):
        market_contract.setAsk(0, [constants.ASK_PRICE, send_any_usd_to_accounts.address], {'from': accounts[2]})
...
def test_should_reject_if_set_bid_from_non_venture_bond(running_launch, accounts, deployed_factory, send_any_usd_to_accounts):
    market_contract = brownie.Market.at(running_launch.launchMarketAddress({'from': accounts[0]}))
    with brownie.reverts("Market: Only VentureBond contract"):
        market_contract.setBid(0,
                                   [constants.BID_PRICE, send_any_usd_to_accounts.address, accounts[2], accounts[2], [0]]

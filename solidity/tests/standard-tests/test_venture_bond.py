...
def test_mint_an_nft(successful_launch, accounts, deployed_factory):
    ...
    for n, inv in enumerate(investors):
        ...
        assert (
            venture_bond_contract.tokenURI(token_id, {"from": inv})
            == constants.GENERIC_NFT_DATA[0]
        )
        assert (
            venture_bond_contract.tokenMetadataHashes(token_id, {"from": inv})
...
def test_set_nft_data_and_mint(successful_launch, accounts, deployed_factory):
    ...
    venture_bond_contract = brownie.VentureBond.at(venture_bond_address)
    launch_contract.setNftDataByIndex(
        0, constants.SPECIAL_NFT_DATA, {"from": accounts[0]}
    )
    launch_contract.claim({"from": special_investor})
    launch_contract.claim({"from": non_special_investor})
    assert (
        venture_bond_contract.tokenURI(0, {"from": special_investor})
        == constants.SPECIAL_NFT_DATA[0]
    )
    assert (
        venture_bond_contract.tokenMetadataHashes(0, {"from": special_investor})
        == "0x" + constants.SPECIAL_NFT_DATA[1]
    )
    assert (
        venture_bond_contract.tokenURI(1, {"from": special_investor})
        == constants.GENERIC_NFT_DATA[0]
    )
    assert (
        venture_bond_contract.tokenMetadataHashes(1, {"from": special_investor})
        == "0x" + constants.GENERIC_NFT_DATA[1]
    )
...
def test_batch_set_nft_data_and_mint(successful_launch, accounts, deployed_factory):
    ...
    venture_bond_contract = brownie.VentureBond.at(venture_bond_address)
    launch_contract.batchSetNftDataByIndex(
    ...
    for n, inv in enumerate(special_investors):
        ...
        assert (
            venture_bond_contract.tokenURI(n, {"from": inv})
            == constants.BATCH_SPECIAL_NFT_DATA[n][0]
        )
        assert (
            venture_bond_contract.tokenMetadataHashes(n, {"from": inv})
            == "0x" + constants.BATCH_SPECIAL_NFT_DATA[n][1]
        )
    launch_contract.claim({"from": non_special_investor})
    assert (
        venture_bond_contract.tokenURI(5, {"from": non_special_investor})
        == constants.GENERIC_NFT_DATA[0]
    )
    assert (
        venture_bond_contract.tokenMetadataHashes(5, {"from": non_special_investor})
        == "0x" + constants.GENERIC_NFT_DATA[1]
    )
...
def test_should_set_ask(minted_launch, accounts, send_1000_stable_to_accounts):
    ...
    for n, inv in enumerate(investors):
        ...
        assert (
            market_contract.currentAskForToken(n, {"from": accounts[0]})[0]
            == constants.ASK_PRICE
        )
        assert (
...
def test_should_accept_bid(
    ...
    assert (
        nft_owner_after_balance == nft_owner_before_balance + constants.BID_PRICE * 0.9
    )
    assert (
        polylaunch_after_balance
        == polylaunch_before_balance + constants.BID_PRICE * 0.1
    )
